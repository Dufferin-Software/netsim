...
class VMConfig:
    ...
    memory_mb: int = 512
    vcpus: int = 1
    tap_devices: List[str] = None  # List of tap interface names
...
class LibvirtVM:
    ...
    def _generate_domain_xml(self) -> str:
        ...
        ET.SubElement(domain, "currentMemory", unit="MiB").text = str(self.config.memory_mb)
        ...
        ET.SubElement(controller, "address", type="pci", domain="0x0000", bus="0x00", slot="0x04", function="0x0")
        ...
        ET.SubElement(disk, "address", type="pci", domain="0x0000", bus="0x00", slot="0x02", function="0x0")
        ...
        qemu_cmd = ET.SubElement(domain, "{http://libvirt.org/schemas/domain/qemu/1.0}commandline")

        # Management interface: user-mode with hostfwd to guest ssh
        if self.config.mgmt_ssh_port:
            ET.SubElement(qemu_cmd, "{http://libvirt.org/schemas/domain/qemu/1.0}arg", value="-netdev")
            ET.SubElement(qemu_cmd, "{http://libvirt.org/schemas/domain/qemu/1.0}arg", value=f"user,id=mgmt,hostfwd=tcp::{self.config.mgmt_ssh_port}-:22")
            ET.SubElement(qemu_cmd, "{http://libvirt.org/schemas/domain/qemu/1.0}arg", value="-device")
            ET.SubElement(qemu_cmd, "{http://libvirt.org/schemas/domain/qemu/1.0}arg", value=f"virtio-net-pci,netdev=mgmt,mac={self._generate_mac(0)},bus=pci.0,addr=0x5")

        # Data interfaces: pre-created tap devices
        for idx, tap_dev in enumerate(self.config.tap_devices, start=1):
            ET.SubElement(qemu_cmd, "{http://libvirt.org/schemas/domain/qemu/1.0}arg", value="-netdev")
            ET.SubElement(qemu_cmd, "{http://libvirt.org/schemas/domain/qemu/1.0}arg", value=f"tap,id=net{idx},ifname={tap_dev},script=no,downscript=no")
            ET.SubElement(qemu_cmd, "{http://libvirt.org/schemas/domain/qemu/1.0}arg", value="-device")
            # Offset addresses to avoid collision with disk (0x2) and mgmt NIC (0x5)
            pci_addr = hex(5 + idx)
            ET.SubElement(qemu_cmd, "{http://libvirt.org/schemas/domain/qemu/1.0}arg", value=f"virtio-net-pci,netdev=net{idx},mac={self._generate_mac(idx)},bus=pci.0,addr={pci_addr}")
    ...
    def start(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["virsh", "domstate", self.config.name],
                capture_output=True,
                timeout=5
            )
            ...
            while not self.is_running():
                if time.time() - start_time > timeout:
                    raise RuntimeError(f"VM {self.config.name} failed to start within {timeout}s")
                time.sleep(0.2)

        except subprocess.CalledProcessError as e:
            stderr = e.stderr.decode() if isinstance(e.stderr, bytes) else str(e.stderr)
            stdout = e.stdout.decode() if isinstance(e.stdout, bytes) else str(e.stdout)
            raise RuntimeError(f"Failed to start VM {self.config.name}: {stderr or stdout}")
    ...
    def stop(self):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to suspend VM {self.config.name}: {e.stderr.decode()}")
    ...
    def get_info(self) -> dict:
        ...
        try:
            result = subprocess.run(
                ["virsh", "dominfo", self.config.name],
                capture_output=True,
                timeout=5
            )
            if result.returncode != 0:
